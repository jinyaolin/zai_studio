...
export async function POST(_req: NextRequest, { params }: { params: { slug: string; sessionId: string } }) {
  if (!isAIConfigured()) {
    return NextResponse.json(
      { error: "AI not configured. Set ZAI_API_KEY in .env" },
      { status: 503 },
    );
  }
  ...
  let session;
  try {
    session = await readSession(workSlug, sessionId);
  } catch {
  ...
  void runAutoContinue(workSlug, sessionId).catch(async (err) => {
    const s = await readSession(workSlug, sessionId);
    s.autoStatus = "failed";
...
async function runAutoContinue(workSlug: string, sessionId: string) {
  for (let i = 0; i < 4; i++) {
    ...
    const result = await generateStage(
      { workSlug, sessionId, stageIndex: i },
  ...
  // All stages done.
  const final = await readSession(workSlug, sessionId);
  final.autoStatus = "done";
  final.autoFinishedAt = new Date().toISOString();
  await writeSession(final);
}
...
export async function GET(_req: NextRequest, { params }: { params: { slug: string; sessionId: string } }) {
  const workSlug = decodeParam(params.slug);
  try {
    const session = await readSession(workSlug, params.sessionId);
    return NextResponse.json({ session });
  } catch {
